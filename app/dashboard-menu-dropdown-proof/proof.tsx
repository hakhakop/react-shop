...
export default function Proof({ shellSettings }: { shellSettings: BuilderShellSettings }) {
  ...
  const host: EmbeddedBuilderHost = { inspectorTarget: target, showInspector: () => setInspect(true), releaseInspector: release, openElements: callback => { insert.current = callback; setLibrary(true); }, openLibrary: () => {} };
  return <main style={{ padding: 20 }}>
    ...
    <aside className="builder-dashboard builder-inspector" style={{ display: "block", position: "relative", width: "100%", maxWidth: 480, marginTop: 40 }}>
      {open ? <MenuDropdownBuilder host={host} item={item} menuId="main" shellSettings={shellSettings} openWordPressMediaPicker={() => {}} onClose={() => setOpen(false)} onApply={dropdownContent => setItem(current => ({ ...current, dropdownContent }))} /> : <button type="button" onClick={() => setOpen(true)}>Open dropdown builder</button>}
    </aside>

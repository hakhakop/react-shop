...
function renderResponsiveConsumerCss(selector: string, policy: ResponsiveBreakpointPolicy) {
  ...
  const previewRules = tierOrder.map((previewTier, index) => {
    ...
    const resetVisibility = `${tiers.map(([tier]) => `${previewRoot} .builder-general-visible-from-${tier}{display:none!important;}${previewRoot} .builder-general-hidden-from-${tier}{display:block!important;}`).join("")}`;

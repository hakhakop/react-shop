...
test("Hero exposes semantic content/actions and keeps builder/frontend parity", async ({ page }) => {
  ...
  await inspector.locator("input[aria-label='Action label']").nth(0).fill("Primary proof");
  await inspector.locator("input[aria-label='Action URL']").nth(0).fill("/primary-proof");
  await inspector.locator("input[aria-label='Action label']").nth(1).fill("Secondary proof");
  await inspector.locator("input[aria-label='Action URL']").nth(1).fill("/secondary-proof");
  await inspector.getByRole("button", { name: "Settings", exact: true }).click();
  await inspector.getByLabel("Button variant").nth(0).getByRole("radio", { name: "Secondary" }).click();
  await inspector.getByLabel("Button size").nth(0).getByRole("radio", { name: "Large" }).click();
  await inspector.getByLabel("Title HTML element").selectOption("h1");
  await inspector.getByLabel("Title style").selectOption("large");

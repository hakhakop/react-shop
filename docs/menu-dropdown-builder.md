...
The slideshow's two product-tag titles and featured MP4 URLs resolve. With explicit user approval, REST exposure was enabled for the Product Tags ACF group (3139), without changing field values. The shared WooCommerce term adapter now supplements GraphQL with tenant-scoped REST ACF fields, including file arrays and attachment IDs. Both imported videos were verified decoded in the browser, with the active video playing. This verifies this fixture's media path, not exhaustive video-provider parity. Imported slides still never substitute demo images.

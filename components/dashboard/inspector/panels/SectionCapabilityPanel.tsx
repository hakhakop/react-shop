...
function QuietCheck({ checked, onChange, label }: { checked: boolean; onChange: (checked: boolean) => void; label: string }) {
  return (
...
function Group({ children }: { title?: string; children: ReactNode }) {
  return <div className="builder-section-inspector-group">{children}</div>;
}
...
export default function SectionCapabilityPanel({ section, tab, update }: Props) {
  ...
  if (tab === "content") {
    return (
      <div className="builder-inspector-stack builder-section-inspector" data-uikit-capability="section-content">
        <Group title="Content">
  ...
  if (tab === "advanced") {
    return (
      <div className="builder-inspector-stack builder-section-inspector" data-uikit-capability="section-advanced">
  ...
  return (
    <div className="builder-inspector-stack builder-section-inspector" data-uikit-capability="section-settings">
      <Group title="Appearance">
        ...
        <InspectorFieldRow label="Preserve Text Color" description="Preserve the text color, for example when using cards. Section overlap is not supported by all styles and may have no visual effect."><QuietCheck checked={section.preserveColor === true} onChange={(preserveColor) => update({ preserveColor })} label="Preserve nested element colors" /></InspectorFieldRow>
      ...
      <Group title="Container">
        ...
        <InspectorFieldRow label="Remove Horizontal Padding"><QuietCheck checked={section.removeHorizontalPadding === true} onChange={(removeHorizontalPadding) => update({ removeHorizontalPadding })} label="Remove left and right padding" /></InspectorFieldRow>
      ...
      <Group title="Height & Alignment">
        ...
        <InspectorFieldRow label="Subtract Height Above Section"><QuietCheck checked={section.subtractHeightAbove === true} onChange={(subtractHeightAbove) => update({ subtractHeightAbove })} label="Subtract preceding layout height" /></InspectorFieldRow>
      ...
      <Group title="Spacing">
        ...
        <InspectorFieldRow label="Remove Top Padding"><QuietCheck checked={section.removeTopPadding === true} onChange={(removeTopPadding) => update({ removeTopPadding, topSpacing: removeTopPadding ? "none" : undefined })} label="Remove top padding" /></InspectorFieldRow>
        <InspectorFieldRow label="Remove Bottom Padding"><QuietCheck checked={section.removeBottomPadding === true} onChange={(removeBottomPadding) => update({ removeBottomPadding, bottomSpacing: removeBottomPadding ? "none" : undefined })} label="Remove bottom padding" /></InspectorFieldRow>
      </Group>
      <Group title="Behavior">
        <InspectorFieldRow label="Overlap Following Section"><QuietCheck checked={section.overlap === true} onChange={(overlap) => update({ overlap })} label="Overlap the next section" /></InspectorFieldRow>
        <InspectorFieldRow label="Header Transparent"><QuietCheck checked={section.headerTransparent === true} onChange={(headerTransparent) => update({ headerTransparent })} label="Overlay section beneath the header" /></InspectorFieldRow>

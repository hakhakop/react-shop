...
export default function ColumnCapabilityPanel({ column, tab, update, openWordPressMediaPicker }: Props) {
  ...
  const stickyEnabled = (column.sticky?.mode ?? "none") !== "none";

  if (tab === "content") return <div className="builder-inspector-stack" data-canonical-owner="BuilderColumn">
    <InspectorFieldRow label="Image"><BuilderImageUrlControl value={column.background?.imageUrl ?? ""} placeholder="http://" onChange={(event) => updateBackground({ imageUrl: event.target.value || undefined })} onChoose={() => openWordPressMediaPicker?.({ title: "Column Image", currentUrl: column.background?.imageUrl, onSelect: (media) => updateBackground({ imageUrl: media.sourceUrl }) })} /></InspectorFieldRow>
    <InspectorFieldRow label="Video"><BuilderImageUrlControl value={column.background?.videoUrl ?? ""} placeholder="http://" onChange={(event) => updateBackground({ videoUrl: event.target.value || undefined })} onChoose={() => openWordPressMediaPicker?.({ title: "Column Video", currentUrl: column.background?.videoUrl, onSelect: (media) => updateBackground({ videoUrl: media.sourceUrl }) })} /></InspectorFieldRow>
  </div>;
  ...
  if (tab === "settings") return <div className="builder-inspector-stack" data-canonical-owner="BuilderColumn">
    ...
    <InspectorFieldRow label="Style"><InspectorSelect value={column.style ?? "none"} options={[{ value: "none", label: "None" }, { value: "default", label: "Default" }, { value: "muted", label: "Muted" }, { value: "primary", label: "Primary" }, { value: "secondary", label: "Secondary" }]} onChange={(style) => update({ style: style === "none" ? undefined : style })} ariaLabel="Column style" /></InspectorFieldRow>
    ...
    <InspectorFieldRow label="Padding"><InspectorSelect value={column.padding ?? "inherit"} disabled={column.style === undefined || column.style === "none"} options={[{ value: "inherit", label: "Default" }, { value: "none", label: "None" }, { value: "small", label: "Small" }, { value: "medium", label: "Medium" }, { value: "large", label: "Large" }, { value: "xlarge", label: "X-Large" }]} onChange={(padding) => update({ padding })} ariaLabel="Column Padding" /></InspectorFieldRow>
    <InspectorFieldRow label="HTML Element"><InspectorSelect value={column.htmlElement ?? "div"} options={["div", "address", "article", "aside", "footer", "header", "hgroup", "main", "nav", "section"].map((value) => ({ value, label: value }))} onChange={(htmlElement) => update({ htmlElement: htmlElement as BuilderLayoutHtmlElement })} ariaLabel="Column HTML Element" /></InspectorFieldRow>
    <InspectorFieldRow label="Position Sticky"><InspectorSelect value={column.sticky?.mode ?? "none"} options={[{ value: "none", label: "None" }, { value: "elements-within-column", label: "Elements within Column" }, { value: "column-within-row", label: "Column within Row" }, { value: "column-within-section", label: "Column within Section" }, { value: "always", label: "Always" }]} onChange={(mode) => updateSticky({ mode })} ariaLabel="Column Sticky Mode" /></InspectorFieldRow>
    ...
    <InspectorFieldRow label="Position Sticky Breakpoint"><InspectorSelect value={column.sticky?.breakpoint ?? ""} disabled={!stickyEnabled} options={[{ value: "", label: "Always" }, { value: "s", label: "Small" }, { value: "m", label: "Medium" }, { value: "l", label: "Large" }, { value: "xl", label: "X-Large" }]} onChange={(breakpoint) => updateSticky({ breakpoint })} ariaLabel="Column sticky breakpoint" /></InspectorFieldRow>

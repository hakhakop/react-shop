...
const supported: { id: Screen; label: string; description: string; enabled?: boolean }[] = [
  { id: "theme", label: "Theme Settings", description: "Import and manage provider page-shell and Header semantics" },
  ...
  { id: "search", label: "Search", description: "Header Search surfaces, icons, placeholders, and borders" },
  { id: "offcanvas", label: "Offcanvas", description: "Dialog panel padding, surface, and overlay" },
  { id: "logo", label: "Logo", description: "Logo typography and text transformation" },
];
...
const editorSections: Partial<Record<Screen, string[]>> = {
  ...
  navbar: ["Surface", "Nav Item", "Item line", "Item line states", "Toggle and subtitle", "Primary and sticky", "Dropdown", "Dropdown nav", "Responsive"],
  search: ["Colors and surfaces", "Default", "Navbar", "Medium and large"],
...
function parseLength(value: unknown) {
  const match = asString(value).trim().match(/^(-?[\d.]+)\s*(px|rem|em|%|vh|vw|ms|s)?$/i);
...
export default function CanonicalGlobalStylesPanel({ shellSettings, updateShellSettings, themeSettings, onImportThemeSettings, onExportThemeSettings }: Props) {
  ...
  const generalItems = supported.filter((item) => item.id === "global" || item.id === "theme");
  const componentOrder: Screen[] = ["accordion", "background", "base", "button", "card", "container", "grid", "heading", "logo", "nav", "navbar", "offcanvas", "search", "section", "visibility"];
    ...
    return <YoothemeColorPaletteProvider colors={themeColorPalette}><div className="builder-global-design-editor" data-testid={`global-editor-${screen}`}>
      ...
      <div className="builder-design-editor-body">
        ...
        <div className="builder-design-editor-fields">
          ...
          {screen === "theme" && <ThemeSettingsPanel embedded themeSettings={themeSettings} onImport={onImportThemeSettings} onExport={onExportThemeSettings} />}
          ...
          {screen === "search" && <SearchGlobalEditor draft={draft} set={set} />}
          {screen === "offcanvas" && <OffcanvasGlobalEditor draft={draft} set={set} />}
          {screen === "logo" && <LogoGlobalEditor draft={draft} set={set} />}
        </div>
      </div>
...
function NavbarGlobalEditor({ draft, set }: { draft: BuilderShellSettings; set: (key: Key, value: string) => void }) {
  return <>
    <Group title="Surface"><BackgroundPaint label="Background" value={draft.navbarBackground} onChange={(value) => set("navbarBackground", value)} /><Select label="Backdrop filter" value={draft.navbarBackdropFilter} options={["none", "blur(5px)", "blur(8px)", "blur(16px)"]} onChange={(value) => set("navbarBackdropFilter", value)} /><Select label="Border mode" value={draft.navbarMode} options={["none", "border"]} onChange={(value) => set("navbarMode", value)} /><Select label="Vertical item dividers" value={draft.navbarModeBorderVertical} options={["none", "partial", "all"]} onChange={(value) => set("navbarModeBorderVertical", value)} /><Length label="Border width" value={draft.navbarBorderWidth} onChange={(value) => set("navbarBorderWidth", value)} /><Color label="Border" value={draft.navbarBorder} onChange={(value) => set("navbarBorder", value)} /><Length label="Dropdown radius" value={draft.navbarDropdownBorderRadius} onChange={(value) => set("navbarDropdownBorderRadius", value)} /><Shadow label="Dropdown shadow" value={draft.navbarDropdownBoxShadow} onChange={(value) => set("navbarDropdownBoxShadow", value)} /></Group>
    <Group title="Nav Item"><Length label="Gap" value={draft.navbarNavGap} onChange={(value) => set("navbarNavGap", value)} /><Length label="Gap @M" value={draft.navbarNavGapMedium} onChange={(value) => set("navbarNavGapMedium", value)} /><Length label="Item height" value={draft.navbarNavItemHeight} onChange={(value) => set("navbarNavItemHeight", value)} /><Length label="Item padding horizontal" value={draft.navbarNavItemPaddingHorizontal} onChange={(value) => set("navbarNavItemPaddingHorizontal", value)} /><Length label="Item padding horizontal @M" value={draft.navbarNavItemPaddingHorizontalMedium} onChange={(value) => set("navbarNavItemPaddingHorizontalMedium", value)} /><Length label="Item font size" value={draft.navbarNavItemFontSize} onChange={(value) => set("navbarNavItemFontSize", value)} /><Color label="Item" value={draft.navbarNavItemColor} onChange={(value) => set("navbarNavItemColor", value)} /><Color label="Item hover" value={draft.navbarNavItemHoverColor} onChange={(value) => set("navbarNavItemHoverColor", value)} /><Color label="Item onclick" value={draft.navbarNavItemOnclickColor} onChange={(value) => set("navbarNavItemOnclickColor", value)} /><Color label="Item active" value={draft.navbarNavItemActiveColor} onChange={(value) => set("navbarNavItemActiveColor", value)} /><Select label="Text transform" value={draft.navbarNavItemTextTransform} options={["none", "uppercase", "lowercase", "capitalize", "inherit"]} onChange={(value) => set("navbarNavItemTextTransform", value)} /><Select label="Parent item indicator" value={draft.navbarDropdownIndicator} options={["none", "chevron"]} onChange={(value) => set("navbarDropdownIndicator", value)} /></Group>
    <Group title="Item line"><Select label="Mode" value={draft.navbarNavItemLineMode ?? "false"} options={["false", "true"]} onChange={(value) => set("navbarNavItemLineMode", value)} /><Select label="Position" value={draft.navbarNavItemLinePositionMode ?? "bottom"} options={["bottom", "top", "left", "right"]} onChange={(value) => set("navbarNavItemLinePositionMode", value)} /><Select label="Slide" value={draft.navbarNavItemLineSlideMode ?? "fixed"} options={["fixed", "center", "left", "right"]} onChange={(value) => set("navbarNavItemLineSlideMode", value)} /><Length label="Height" value={draft.navbarNavItemLineHeight ?? "1px"} onChange={(value) => set("navbarNavItemLineHeight", value)} /><Length label="Horizontal margin" value={draft.navbarNavItemLineMarginHorizontal ?? "0"} onChange={(value) => set("navbarNavItemLineMarginHorizontal", value)} units={["px", "rem", "%"]} /><Length label="Vertical margin" value={draft.navbarNavItemLineMarginVertical ?? "-1px"} onChange={(value) => set("navbarNavItemLineMarginVertical", value)} units={["px", "rem", "%"]} /><Length label="Border radius" value={draft.navbarNavItemLineBorderRadius ?? "0"} onChange={(value) => set("navbarNavItemLineBorderRadius", value)} /><Length label="Transition duration" value={draft.navbarNavItemLineTransitionDuration ?? "0.1s"} onChange={(value) => set("navbarNavItemLineTransitionDuration", value)} units={["s", "ms"]} /><Length label="Base opacity" value={draft.navbarNavItemLineOpacity ?? "1"} onChange={(value) => set("navbarNavItemLineOpacity", value)} units={["", "%"]} /><BackgroundPaint label="Base line paint" value={draft.navbarNavItemLineGradient ?? "transparent"} onChange={(value) => set("navbarNavItemLineGradient", value)} /></Group>
    <Group title="Item line states"><Length label="Hover height" value={draft.navbarNavItemLineHoverHeight ?? draft.navbarNavItemLineHeight ?? "1px"} onChange={(value) => set("navbarNavItemLineHoverHeight", value)} /><BackgroundPaint label="Hover paint" value={draft.navbarNavItemLineHoverBackground ?? "currentColor"} onChange={(value) => set("navbarNavItemLineHoverBackground", value)} /><Length label="Hover opacity" value={draft.navbarNavItemLineHoverOpacity ?? "1"} onChange={(value) => set("navbarNavItemLineHoverOpacity", value)} units={["", "%"]} /><Length label="Onclick height" value={draft.navbarNavItemLineOnclickHeight ?? draft.navbarNavItemLineHeight ?? "1px"} onChange={(value) => set("navbarNavItemLineOnclickHeight", value)} /><BackgroundPaint label="Onclick paint" value={draft.navbarNavItemLineOnclickBackground ?? "currentColor"} onChange={(value) => set("navbarNavItemLineOnclickBackground", value)} /><Length label="Onclick opacity" value={draft.navbarNavItemLineOnclickOpacity ?? "1"} onChange={(value) => set("navbarNavItemLineOnclickOpacity", value)} units={["", "%"]} /><Length label="Active height" value={draft.navbarNavItemLineActiveHeight ?? draft.navbarNavItemLineHeight ?? "1px"} onChange={(value) => set("navbarNavItemLineActiveHeight", value)} /><BackgroundPaint label="Active paint" value={draft.navbarNavItemLineActiveBackground ?? "currentColor"} onChange={(value) => set("navbarNavItemLineActiveBackground", value)} /><Length label="Active opacity" value={draft.navbarNavItemLineActiveOpacity ?? "1"} onChange={(value) => set("navbarNavItemLineActiveOpacity", value)} units={["", "%"]} /></Group>
    <Group title="Toggle and subtitle"><Color label="Toggle" value={draft.navbarToggleColor} onChange={(value) => set("navbarToggleColor", value)} /><Color label="Toggle hover" value={draft.navbarToggleHoverColor} onChange={(value) => set("navbarToggleHoverColor", value)} /><Length label="Subtitle font size" value={draft.navbarSubtitleFontSize} onChange={(value) => set("navbarSubtitleFontSize", value)} /><YoothemeFontPicker label="Subtitle font family" value={draft.navbarSubtitleFontFamily ?? "inherit"} onChange={(value) => set("navbarSubtitleFontFamily", value)} /><Select label="Subtitle weight" value={draft.navbarSubtitleFontWeight} options={["normal", "400", "500", "600", "700"]} onChange={(value) => set("navbarSubtitleFontWeight", value)} /><Select label="Subtitle transform" value={draft.navbarSubtitleTextTransform} options={["none", "uppercase", "lowercase", "capitalize"]} onChange={(value) => set("navbarSubtitleTextTransform", value)} /><Color label="Subtitle" value={draft.navbarSubtitleColor} onChange={(value) => set("navbarSubtitleColor", value)} /></Group>
    ...
    <Group title="Generic Dropdown surface"><Length label="Padding" value={draft.dropdownPadding} onChange={(value) => set("dropdownPadding", value)} /><BackgroundPaint label="Background" value={draft.dropdownBackground} onChange={(value) => set("dropdownBackground", value)} /><Color label="Text" value={draft.dropdownColor} onChange={(value) => set("dropdownColor", value)} /><Select label="Color mode" value={draft.dropdownColorMode} options={["light", "dark"]} onChange={(value) => set("dropdownColorMode", value)} /><Length label="Dropbar margin" value={draft.dropdownDropbarMargin} onChange={(value) => set("dropdownDropbarMargin", value)} /><Length label="Dropbar padding top" value={draft.dropdownDropbarPaddingTop} onChange={(value) => set("dropdownDropbarPaddingTop", value)} /><Length label="Dropbar large padding top" value={draft.dropdownDropbarLargePaddingTop} onChange={(value) => set("dropdownDropbarLargePaddingTop", value)} /></Group>
    <Group title="Generic Dropdown nav"><Length label="Font size" value={draft.dropdownNavFontSize} onChange={(value) => set("dropdownNavFontSize", value)} /><Length label="Item padding vertical" value={draft.dropdownNavItemPaddingVertical} onChange={(value) => set("dropdownNavItemPaddingVertical", value)} /><Color label="Item hover" value={draft.dropdownNavItemHoverColor} onChange={(value) => set("dropdownNavItemHoverColor", value)} /><Length label="Subtitle font size" value={draft.dropdownNavSubtitleFontSize} onChange={(value) => set("dropdownNavSubtitleFontSize", value)} /><Color label="Subtitle" value={draft.dropdownNavSubtitleColor} onChange={(value) => set("dropdownNavSubtitleColor", value)} /><Color label="Header" value={draft.dropdownNavHeaderColor} onChange={(value) => set("dropdownNavHeaderColor", value)} /><Color label="Divider" value={draft.dropdownNavDividerBorder} onChange={(value) => set("dropdownNavDividerBorder", value)} /><Color label="Sublist item hover" value={draft.dropdownNavSublistItemHoverColor} onChange={(value) => set("dropdownNavSublistItemHoverColor", value)} /></Group>
    <Group title="Dropbar surface"><Length label="Padding top" value={draft.dropbarPaddingTop} onChange={(value) => set("dropbarPaddingTop", value)} /><BackgroundPaint label="Background" value={draft.dropbarBackground} onChange={(value) => set("dropbarBackground", value)} /><Color label="Text" value={draft.dropbarColor} onChange={(value) => set("dropbarColor", value)} /><Select label="Color mode" value={draft.dropbarColorMode} options={["light", "dark"]} onChange={(value) => set("dropbarColorMode", value)} /></Group>
    <Group title="Dropdown nav"><Length label="Font size" value={draft.navbarDropdownNavFontSize} onChange={(value) => set("navbarDropdownNavFontSize", value)} /><Length label="Item padding vertical" value={draft.navbarDropdownNavItemPaddingVertical} onChange={(value) => set("navbarDropdownNavItemPaddingVertical", value)} /><Color label="Item" value={draft.navbarDropdownNavItemColor} onChange={(value) => set("navbarDropdownNavItemColor", value)} /><Color label="Item hover" value={draft.navbarDropdownNavItemHoverColor} onChange={(value) => set("navbarDropdownNavItemHoverColor", value)} /><Color label="Item active" value={draft.navbarDropdownNavItemActiveColor} onChange={(value) => set("navbarDropdownNavItemActiveColor", value)} /><Length label="Subtitle font size" value={draft.navbarDropdownNavSubtitleFontSize} onChange={(value) => set("navbarDropdownNavSubtitleFontSize", value)} /><YoothemeFontPicker label="Subtitle font family" value={draft.navbarDropdownNavSubtitleFontFamily ?? "inherit"} onChange={(value) => set("navbarDropdownNavSubtitleFontFamily", value)} /><Select label="Subtitle weight" value={draft.navbarDropdownNavSubtitleFontWeight} options={["normal", "400", "500", "600", "700"]} onChange={(value) => set("navbarDropdownNavSubtitleFontWeight", value)} /><Select label="Subtitle transform" value={draft.navbarDropdownNavSubtitleTextTransform} options={["none", "uppercase", "lowercase", "capitalize"]} onChange={(value) => set("navbarDropdownNavSubtitleTextTransform", value)} /><Color label="Subtitle" value={draft.navbarDropdownNavSubtitleColor} onChange={(value) => set("navbarDropdownNavSubtitleColor", value)} /><Color label="Sublist item hover" value={draft.navbarDropdownNavSublistItemHoverColor} onChange={(value) => set("navbarDropdownNavSublistItemHoverColor", value)} /><Color label="Sublist item active" value={draft.navbarDropdownNavSublistItemActiveColor} onChange={(value) => set("navbarDropdownNavSublistItemActiveColor", value)} /></Group>
...
function SearchGlobalEditor({ draft, set }: { draft: BuilderShellSettings; set: (key: Key, value: string) => void }) {
  return <>
    <Group title="Colors and surfaces"><Color label="Placeholder" value={draft.searchPlaceholderColor} onChange={(value) => set("searchPlaceholderColor", value)} /><Color label="Icon" value={draft.searchIconColor} onChange={(value) => set("searchIconColor", value)} /><Color label="Toggle" value={draft.searchToggleColor} onChange={(value) => set("searchToggleColor", value)} /><Color label="Toggle hover" value={draft.searchToggleHoverColor} onChange={(value) => set("searchToggleHoverColor", value)} /><BackgroundPaint label="Default background" value={draft.searchDefaultBackground} onChange={(value) => set("searchDefaultBackground", value)} /><BackgroundPaint label="Navbar background" value={draft.searchNavbarBackground} onChange={(value) => set("searchNavbarBackground", value)} /></Group>
    <Group title="Default"><Length label="Border width" value={draft.searchDefaultBorderWidth} onChange={(value) => set("searchDefaultBorderWidth", value)} /><Color label="Border" value={draft.searchDefaultBorder} onChange={(value) => set("searchDefaultBorder", value)} /><Color label="Focus border" value={draft.searchDefaultFocusBorder} onChange={(value) => set("searchDefaultFocusBorder", value)} /></Group>
    <Group title="Navbar"><Length label="Border width" value={draft.searchNavbarBorderWidth} onChange={(value) => set("searchNavbarBorderWidth", value)} /><Color label="Border" value={draft.searchNavbarBorder} onChange={(value) => set("searchNavbarBorder", value)} /><Color label="Focus border" value={draft.searchNavbarFocusBorder} onChange={(value) => set("searchNavbarFocusBorder", value)} /></Group>
    <Group title="Medium and large"><Length label="Medium border width" value={draft.searchMediumBorderWidth} onChange={(value) => set("searchMediumBorderWidth", value)} /><Color label="Medium border" value={draft.searchMediumBorder} onChange={(value) => set("searchMediumBorder", value)} /><Color label="Medium focus border" value={draft.searchMediumFocusBorder} onChange={(value) => set("searchMediumFocusBorder", value)} /><Length label="Large border width" value={draft.searchLargeBorderWidth} onChange={(value) => set("searchLargeBorderWidth", value)} /><Color label="Large border" value={draft.searchLargeBorder} onChange={(value) => set("searchLargeBorder", value)} /><Color label="Large focus border" value={draft.searchLargeFocusBorder} onChange={(value) => set("searchLargeFocusBorder", value)} /></Group>
  </>;
}
function OffcanvasGlobalEditor({ draft, set }: { draft: BuilderShellSettings; set: (key: Key, value: string) => void }) {
  return <><Group title="Panel"><Length label="Vertical padding" value={draft.offcanvasBarPaddingVertical} onChange={(value) => set("offcanvasBarPaddingVertical", value)} /><Length label="Horizontal padding" value={draft.offcanvasBarPaddingHorizontal} onChange={(value) => set("offcanvasBarPaddingHorizontal", value)} /><BackgroundPaint label="Background" value={draft.offcanvasBarBackground} onChange={(value) => set("offcanvasBarBackground", value)} /><Select label="Color mode" value={draft.offcanvasBarColorMode} options={["light", "dark"]} onChange={(value) => set("offcanvasBarColorMode", value)} /></Group><Group title="Overlay"><BackgroundPaint label="Overlay background" value={draft.offcanvasOverlayBackground} onChange={(value) => set("offcanvasOverlayBackground", value)} /></Group></>;
}
function LogoGlobalEditor({ draft, set }: { draft: BuilderShellSettings; set: (key: Key, value: string) => void }) {
  return <Group title="Typography"><Length label="Font size" value={draft.logoFontSize} onChange={(value) => set("logoFontSize", value)} /><Select label="Text transform" value={draft.logoTextTransform} options={["none", "uppercase", "lowercase", "capitalize"]} onChange={(value) => set("logoTextTransform", value)} /></Group>;
}

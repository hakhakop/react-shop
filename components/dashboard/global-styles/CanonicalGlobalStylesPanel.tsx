...
const editorSections: Partial<Record<Screen, string[]>> = {
  global: ["Typography", "Primary", "Secondary", "Tertiary", "Colors", "Borders", "Box shadows", "Spacing", "Controls", "Layout infrastructure", "Containers"],
...
function GlobalEditor({ draft, set }: { draft: BuilderShellSettings; set: (key: Key, value: string) => void }) { return <>
  ...
  <ColorGroup draft={draft} set={set} /><BorderGroup draft={draft} set={set} /><ShadowGroup draft={draft} set={set} /><ShellSpacingGroup draft={draft} set={set} /><SpacingGroup draft={draft} set={set} /><ControlGroup draft={draft} set={set} /><InfrastructureGroup draft={draft} set={set} /><ContainerGroup draft={draft} set={set} /></>; }
...
function InfrastructureGroup({ draft, set }: { draft: BuilderShellSettings; set: (key: Key, value: string) => void }) { return <Group title="Layout infrastructure"><Length label="Z index" value={draft.globalZIndex} onChange={(value) => set("globalZIndex", value)} units={[""]} />{([["breakpointSmall", "Small breakpoint"], ["breakpointMedium", "Medium breakpoint"], ["breakpointLarge", "Large breakpoint"], ["breakpointXLarge", "Xlarge breakpoint"]] as [Key, string][]).map(([key, label]) => <Length key={key} label={label} value={draft[key]} onChange={(value) => set(key, value)} />)}</Group>; }

...
import { resolveYoothemeLess, type YoothemeImportRow, type YoothemeLessSource, type YoothemeSemanticPreset } from "@/lib/yoothemeLessImporter";
...
async function readSources(fileList: FileList | null): Promise<YoothemeLessSource[]> {
  ...
  const entries = [
    ...
    ["master-devstack/styles/light-blue.less", source("light-blue.less", "master-devstack/styles/light-blue.less"), 2],
...
export default function YoothemeImportPanel({ design, shellSettings, updateDesign, updateShellSettings }: Props) {
  ...
  const preview = async (files: FileList | null) => {
    setReading(true);
    setError("");
    try {
      ...
      setPreset(resolveYoothemeLess(sources));
    } catch (cause) {
  ...
  return (
    <div className="builder-global-styles-group" data-testid="yootheme-import-panel">
      ...
      <p className="builder-shell-note">Choose DevStack.zip, or select the four LESS layers. Home.json is intentionally not accepted by this importer.</p>
      <label className="builder-field"><span>DevStack LESS files</span><input type="file" accept=".zip,.less" multiple onChange={(event) => void preview(event.target.files)} /></label>
      ...
      {preset ? (
        <>
          ...
          <button type="button" className="builder-global-preset-apply" onClick={apply}>Apply DevStack Light Blue</button>
